...
import (
	...
	ljRBvXJ "github.com/paloaltonetworks/sase-go/netsec/schema/objects/application/groups"
)
...
func (c *Client) Create(ctx context.Context, input CreateInput) (ljRBvXJ.Config, error) {
	// Variables.
	var err error
	var ans ljRBvXJ.Config
...
func (c *Client) Delete(ctx context.Context, input DeleteInput) (ljRBvXJ.Config, error) {
	// Variables.
	var err error
	var ans ljRBvXJ.Config
...
type ListOutput struct {
	Data   []ljRBvXJ.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]ljRBvXJ.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]ljRBvXJ.Config)
	...
	listing := make([]ljRBvXJ.Config, 0, len(items))
	for key := range items {
...
type PutInputConfig struct {
	Entry ljRBvXJ.Config `json:"entry,omitempty"`
}
...
// Method: get
// URI: /sse/config/v1/application-groups/{id}
func (c *Client) Read(ctx context.Context, input ReadInput) (ljRBvXJ.Config, error) {
	// Variables.
	var err error
	var ans ljRBvXJ.Config
	path := "/sse/config/v1/application-groups/{id}"
