...
import (
    ...
    jKoxElO "github.com/paloaltonetworks/sase-go/netsec/schema/authentication/settings"
)
...
// Method: post
// URI: /sse/config/v1/mobile-agent/authentication-settings
func (c *Client) Create(ctx context.Context, input CreateInput)  (jKoxElO.Config, error) {
    // Variables.
    var err error
    var ans jKoxElO.Config
    path := "/sse/config/v1/mobile-agent/authentication-settings"
...
type ListOutput struct {
    Data []jKoxElO.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
    var ans ListOutput
    var err error
    var items map[string] jKoxElO.Config
    ...
    for {
        ...
        total := int(ans.Total)
        items = make(map[string] jKoxElO.Config)
    ...
    listing := make([]jKoxElO.Config, 0, len(items))
    for key := range items {
