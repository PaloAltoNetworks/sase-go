...
import (
    ...
    uuDOzKc "github.com/paloaltonetworks/sase-go/netsec/schema/mobile/agent/locations"
)
...
type ListOutput struct {
    Data []uuDOzKc.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
    var ans ListOutput
    var err error
    var items map[string] uuDOzKc.Config
    ...
    for {
        ...
        total := int(ans.Total)
        items = make(map[string] uuDOzKc.Config)
    ...
    listing := make([]uuDOzKc.Config, 0, len(items))
    for key := range items {
