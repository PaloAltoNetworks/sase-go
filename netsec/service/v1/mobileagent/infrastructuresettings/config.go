...
import (
	...
	usEbezS "github.com/paloaltonetworks/sase-go/netsec/schema/mobile/agent/infrastructure/settings"
)
...
func (c *Client) Create(ctx context.Context, input CreateInput) (usEbezS.Config, error) {
	// Variables.
	var err error
	var ans usEbezS.Config
...
type ListOutput struct {
	Data   []usEbezS.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]usEbezS.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]usEbezS.Config)
	...
	listing := make([]usEbezS.Config, 0, len(items))
	for key := range items {
