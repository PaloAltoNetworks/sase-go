...
import (
	...
	pRvGXnY "github.com/paloaltonetworks/sase-go/netsec/schema/trusted/certificate/authorities"
)
...
type ListOutput struct {
	Data   []pRvGXnY.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]pRvGXnY.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]pRvGXnY.Config)
	...
	listing := make([]pRvGXnY.Config, 0, len(items))
	for key := range items {
