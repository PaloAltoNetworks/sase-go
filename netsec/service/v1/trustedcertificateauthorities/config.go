...
import (
    ...
    nyyKgji "github.com/paloaltonetworks/sase-go/netsec/schema/trusted/certificate/authorities"
)
...
type ListOutput struct {
    Data []nyyKgji.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
    var ans ListOutput
    var err error
    var items map[string] nyyKgji.Config
    ...
    for {
        ...
        total := int(ans.Total)
        items = make(map[string] nyyKgji.Config)
    ...
    listing := make([]nyyKgji.Config, 0, len(items))
    for key := range items {
