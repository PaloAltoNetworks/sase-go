...
import (
    ...
    vouiOqW "github.com/paloaltonetworks/sase-go/netsec/schema/bgp/routing"
)
...
type ListOutput struct {
    Data []vouiOqW.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
    var ans ListOutput
    var err error
    var items map[string] vouiOqW.Config
    ...
    for {
        ...
        total := int(ans.Total)
        items = make(map[string] vouiOqW.Config)
    ...
    listing := make([]vouiOqW.Config, 0, len(items))
    for key := range items {
