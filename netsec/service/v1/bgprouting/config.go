...
import (
	...
	fNsbiUH "github.com/paloaltonetworks/sase-go/netsec/schema/bgp/routing"
)
...
type ListOutput struct {
	Data   []fNsbiUH.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]fNsbiUH.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]fNsbiUH.Config)
	...
	listing := make([]fNsbiUH.Config, 0, len(items))
	for key := range items {
