...
// []string{"netsec", "service", "v1", "ocspresponder"}
...
import (
	...
	joFnDrm "github.com/paloaltonetworks/sase-go/netsec/schema/ocsp/responder"
)
...
type CreateInput struct {
	Folder string
	Config joFnDrm.Config
}

// Create creates the specified object.
//
// Method: post
// URI: /sse/config/v1/ocsp-responder
func (c *Client) Create(ctx context.Context, input CreateInput) (joFnDrm.Config, error) {
	// Variables.
	var err error
	var ans joFnDrm.Config
	path := "/sse/config/v1/ocsp-responder"
...
//
// Method: delete
// URI: /sse/config/v1/ocsp-responder/{id}
func (c *Client) Delete(ctx context.Context, input DeleteInput) (joFnDrm.Config, error) {
	// Variables.
	var err error
	var ans joFnDrm.Config
	path := "/sse/config/v1/ocsp-responder/{id}"
...
type ListOutput struct {
	Data   []joFnDrm.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]joFnDrm.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]joFnDrm.Config)
	...
	listing := make([]joFnDrm.Config, 0, len(items))
	for key := range items {
...
// Method: get
// URI: /sse/config/v1/ocsp-responder/{id}
func (c *Client) Read(ctx context.Context, input ReadInput) (joFnDrm.Config, error) {
	// Variables.
	var err error
	var ans joFnDrm.Config
	path := "/sse/config/v1/ocsp-responder/{id}"
...
// Method: put
// URI: /sse/config/v1/ocsp-responder/{id}
func (c *Client) Update(ctx context.Context, input UpdateInput) (joFnDrm.Config, error) {
	// Variables.
	var err error
	var ans joFnDrm.Config
	path := "/sse/config/v1/ocsp-responder/{id}"
