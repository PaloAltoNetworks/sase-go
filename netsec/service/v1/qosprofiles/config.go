...
// []string{"netsec", "service", "v1", "qosprofiles"}
...
import (
	...
	kllvJfd "github.com/paloaltonetworks/sase-go/netsec/schema/qos/profiles"
)
...
type CreateInput struct {
	Folder string
	Config kllvJfd.Config
}

// Create creates the specified object.
//
// Method: post
// URI: /sse/config/v1/qos-profiles
func (c *Client) Create(ctx context.Context, input CreateInput) (kllvJfd.Config, error) {
	// Variables.
	var err error
	var ans kllvJfd.Config
	path := "/sse/config/v1/qos-profiles"
...
//
// Method: delete
// URI: /sse/config/v1/qos-profiles/{id}
func (c *Client) Delete(ctx context.Context, input DeleteInput) (kllvJfd.Config, error) {
	// Variables.
	var err error
	var ans kllvJfd.Config
	path := "/sse/config/v1/qos-profiles/{id}"
...
type ListOutput struct {
	Data   []kllvJfd.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]kllvJfd.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]kllvJfd.Config)
	...
	listing := make([]kllvJfd.Config, 0, len(items))
	for key := range items {
...
// Method: get
// URI: /sse/config/v1/qos-profiles/{id}
func (c *Client) Read(ctx context.Context, input ReadInput) (kllvJfd.Config, error) {
	// Variables.
	var err error
	var ans kllvJfd.Config
	path := "/sse/config/v1/qos-profiles/{id}"
...
// Method: put
// URI: /sse/config/v1/qos-profiles/{id}
func (c *Client) Update(ctx context.Context, input UpdateInput) (kllvJfd.Config, error) {
	// Variables.
	var err error
	var ans kllvJfd.Config
	path := "/sse/config/v1/qos-profiles/{id}"
