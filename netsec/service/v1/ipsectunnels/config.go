...
// []string{"netsec", "service", "v1", "ipsectunnels"}
...
import (
	...
	tOwfYRO "github.com/paloaltonetworks/sase-go/netsec/schema/ipsec/tunnels"
)
...
type CreateInput struct {
	Folder string
	Config tOwfYRO.Config
}

// Create creates the specified object.
//
// Method: post
// URI: /sse/config/v1/ipsec-tunnels
func (c *Client) Create(ctx context.Context, input CreateInput) (tOwfYRO.Config, error) {
	// Variables.
	var err error
	var ans tOwfYRO.Config
	path := "/sse/config/v1/ipsec-tunnels"
...
//
// Method: delete
// URI: /sse/config/v1/ipsec-tunnels/{id}
func (c *Client) Delete(ctx context.Context, input DeleteInput) (tOwfYRO.Config, error) {
	// Variables.
	var err error
	var ans tOwfYRO.Config
...
type ListOutput struct {
	Data   []tOwfYRO.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]tOwfYRO.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]tOwfYRO.Config)
	...
	listing := make([]tOwfYRO.Config, 0, len(items))
	for key := range items {
...
// Method: get
// URI: /sse/config/v1/ipsec-tunnels/{id}
func (c *Client) Read(ctx context.Context, input ReadInput) (tOwfYRO.Config, error) {
	// Variables.
	var err error
	var ans tOwfYRO.Config
	path := "/sse/config/v1/ipsec-tunnels/{id}"
...
// Method: put
// URI: /sse/config/v1/ipsec-tunnels/{id}
func (c *Client) Update(ctx context.Context, input UpdateInput) (tOwfYRO.Config, error) {
	// Variables.
	var err error
	var ans tOwfYRO.Config
	path := "/sse/config/v1/ipsec-tunnels/{id}"
