...
import (
	...
	xcweycI "github.com/paloaltonetworks/sase-go/netsec/schema/auto/tag/actions"
)
...
// Method: post
// URI: /sse/config/v1/auto-tag-actions
func (c *Client) Create(ctx context.Context, input CreateInput) (xcweycI.Config, error) {
	// Variables.
	var err error
	var ans xcweycI.Config
	path := "/sse/config/v1/auto-tag-actions"
...
type ListOutput struct {
	Data   []xcweycI.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]xcweycI.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]xcweycI.Config)
	...
	listing := make([]xcweycI.Config, 0, len(items))
	for key := range items {
