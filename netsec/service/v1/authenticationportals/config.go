...
import (
	...
	sLUkZzJ "github.com/paloaltonetworks/sase-go/netsec/schema/authentication/portals"
)
...
func (c *Client) Create(ctx context.Context, input CreateInput) (sLUkZzJ.Config, error) {
	// Variables.
	var err error
	var ans sLUkZzJ.Config
...
func (c *Client) Delete(ctx context.Context, input DeleteInput) (sLUkZzJ.Config, error) {
	// Variables.
	var err error
	var ans sLUkZzJ.Config
...
type ListOutput struct {
	Data   []sLUkZzJ.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]sLUkZzJ.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]sLUkZzJ.Config)
	...
	listing := make([]sLUkZzJ.Config, 0, len(items))
	for key := range items {
...
// Method: put
// URI: /sse/config/v1/authentication-portals/{id}
func (c *Client) Update(ctx context.Context, input UpdateInput) (sLUkZzJ.Config, error) {
	// Variables.
	var err error
	var ans sLUkZzJ.Config
	path := "/sse/config/v1/authentication-portals/{id}"
