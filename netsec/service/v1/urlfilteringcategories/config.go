...
import (
	...
	dWyIppf "github.com/paloaltonetworks/sase-go/netsec/schema/url/filtering/categories"
)
...
type ListOutput struct {
	Data   []dWyIppf.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]dWyIppf.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]dWyIppf.Config)
	...
	listing := make([]dWyIppf.Config, 0, len(items))
	for key := range items {
