...
import (
	...
	yxbdjfr "github.com/paloaltonetworks/sase-go/netsec/schema/jobs"
)
...
type ListOutput struct {
	Data   []yxbdjfr.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]yxbdjfr.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]yxbdjfr.Config)
	...
	listing := make([]yxbdjfr.Config, 0, len(items))
	for key := range items {
...
func (c *Client) Read(ctx context.Context, input ReadInput) (yxbdjfr.Config, error) {
	// Variables.
	var err error
	var ans yxbdjfr.Config
