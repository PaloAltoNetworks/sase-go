...
import (
	...
	qsbbGtI "github.com/paloaltonetworks/sase-go/netsec/schema/edit/shared/infrastructure/settings"
	xgwCFDH "github.com/paloaltonetworks/sase-go/netsec/schema/shared/infrastructure/settings"
)
...
type ListOutput struct {
	Data   []xgwCFDH.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
	var ans ListOutput
	var err error
	var items map[string]xgwCFDH.Config
	...
	for {
		...
		total := int(ans.Total)
		items = make(map[string]xgwCFDH.Config)
	...
	listing := make([]xgwCFDH.Config, 0, len(items))
	for key := range items {
