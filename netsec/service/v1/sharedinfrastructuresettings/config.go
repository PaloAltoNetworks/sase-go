...
import (
    ...
    gHJDesq "github.com/paloaltonetworks/sase-go/netsec/schema/shared/infrastructure/settings"
    mBDljFP "github.com/paloaltonetworks/sase-go/netsec/schema/edit/shared/infrastructure/settings"
)
...
type ListOutput struct {
    Data []gHJDesq.Config `json:"data,omitempty"`
...
func (c *Client) listAll(ctx context.Context, input ListInput) (ListOutput, error) {
    var ans ListOutput
    var err error
    var items map[string] gHJDesq.Config
    ...
    for {
        ...
        total := int(ans.Total)
        items = make(map[string] gHJDesq.Config)
    ...
    listing := make([]gHJDesq.Config, 0, len(items))
    for key := range items {
